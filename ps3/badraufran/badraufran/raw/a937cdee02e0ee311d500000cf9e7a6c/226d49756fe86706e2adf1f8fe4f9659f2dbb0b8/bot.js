...
function MarbleImage() {
  ...
  this.colourPaletteUrl = 'http://crossorigin.me/http://www.colourlovers.com/api/palettes/top?format=json&numResults=1&resultOffset=';
  this.colourPalette = [];
  this.crayolaColours;
  this.artNames;
  this.dominantColour;
  ...
  this.loadColorPalette = function() {
    ...
    // Send GET request to load JSON
    var self = this;
    loadJSON(this.colourPaletteUrl, function(data) {
      // Set colourPalette with the 5 colours
      var colours = data[0].colors;
      for (var i = 0; i < colours.length; i++) {
        self.colourPalette[i] = colours[i];
      }
      self.sortColourPalette();
    }, function(res) {
      console.log('Error with COLOURlovers API: ' + res);
    });
  }
