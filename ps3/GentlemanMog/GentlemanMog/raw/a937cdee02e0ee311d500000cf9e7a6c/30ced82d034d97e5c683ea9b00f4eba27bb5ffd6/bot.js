...
//Arraylist of current pokemon details
  ...
  this.setup = function() {
    ...
    var language = "en";
      ...
      var is_found = false;
      var num_loops = 0;
     // while (is_found === false && num_loops < 3) {
        this.rngVersion = random(closed_versions);

        // if (!is_found) {
        //     console.log("Match not found: " + this.rngVersion);
    ...
    for(var i = 0; i < dexEntry.length; i++){
      ...
      //JSON Variables for this.pokemon array
      ...
      this.pokemon.push(data);
        ...
        //If the rng closed data equals a version in the array and its languege is english display dex entry
          if(this.rngVersion == this.pokemon[i][0] && this.pokemon[i][1] == language){
            ...
            //if array contains an entry with "" or contains nothing;
            if (detailSplit[rngEntry] === null || detailSplit[rngEntry] == "") {
              // console.log("Found empty entry, retrying");
    ...
    //if the version is not in the array then prin
    if (is_found === false) {
      this.rngVersion = random(closed_versions);
       detail = "Has no Data entry in " + this.rngVersion + "."; 
    }else{
