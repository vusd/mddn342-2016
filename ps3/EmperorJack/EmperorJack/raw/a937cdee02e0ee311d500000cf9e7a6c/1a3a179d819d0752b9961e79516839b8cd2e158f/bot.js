...
function bot() {
  ...
  // Setups up the data object that contains data derived from a random news article
  ...
  // Gets all the noun pharses for the given text from the Text-API
  ...
  // Gets all the word tags for the given text from the Text-API
  this.getWordText = function(text, nounPhrases) {
    ...
    .done(function(data) {
      ...
      // Process the noun phrases and word tag data
      bot.textAnalysisRecieved(nounPhrases, data);
    })
  }

  // Called when the noun phrases and word tag data has been recieved
  this.textAnalysisRecieved = function(nounPhrases, wordTags) {

    // Use the word tags to identify the nouns in the text
    var nouns = [];
  ...
  // Checks if the given phrase (string of multiple words) contains any of the given nouns
  ...
  this.setup = function() {
    // Nothing to do here. Preload handles all of the setup.
  }
  ...
  this.respond = function() {
    ...
    // Image and message have now been successfully generated
    this.done = true;

    return message;
  }

  // Draw the image for the given quote type
  ...
  // Generate the message text for the given quote type
